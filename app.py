...
AUDIT_LOCK    = AUDIT_FILE + ".lock"
LOCK_TIMEOUT  = 5   # seconds — fail loudly rather than hang the whole app if a lock is stuck
FACES_DIR     = "faces"
...
def _get_mongo_client():
    """
    Open ONE MongoClient for the life of the process and reuse it across every
    session/rerun. Previously every load_db/save_db/load_audit/add_audit_event
    call opened a brand-new client + did a fresh `ping`, which is slow and
    burns through connection limits fast on Atlas's free tier once more than
    a couple of users are active.
    """
...
def load_db() -> dict:
    ...
    if not os.path.exists(DB_FILE):
        return {}
    try:
        with FileLock(DB_LOCK, timeout=LOCK_TIMEOUT):
            with open(DB_FILE) as f:
                return json.load(f)
    except Timeout:
        st.warning("⚠️ User database is busy right now — please retry in a moment.")
        return {}
...
def save_db(db: dict):
    ...
    try:
        ...
        st.error("🚫 Could not save — user database is locked by another request. Please retry.")
...
def load_audit() -> list:
    ...
    if not os.path.exists(AUDIT_FILE):
        return []
    try:
        with FileLock(AUDIT_LOCK, timeout=LOCK_TIMEOUT):
...
def save_audit(log: list):
    ...
    try:
        ...
        pass  # audit log is best-effort; don't block the user's actual request over this
...
def add_audit_event(username, event, score=None, ip="browser", success=None):
    ...
    if audit_col is not None:
        ...
        return
    # load + append + save under a single lock hold so two concurrent logins
    # can't both read the same log, append separately, and have the second
    # write clobber the first one's entry.
    try:
        with FileLock(AUDIT_LOCK, timeout=LOCK_TIMEOUT):
            log = json.load(open(AUDIT_FILE)) if os.path.exists(AUDIT_FILE) else []
            log.append(entry)
            with open(AUDIT_FILE, "w") as f:
                json.dump(log[-500:], f, indent=2)
    except Timeout:
        pass  # best-effort; don't block login/register over a busy audit log
